...
spec = do
  ...
  describe "function" $ do
    ...
    parses "one argument"       "#([a int] 5)"        (AFunc () [("a", AIntTy)] (AInt () 5))
    parses "multiple arguments" "#([a int b unit] 5)" (AFunc () [("a", AIntTy), ("b", AUnitTy)] (AInt () 5))

  describe "application" $ do
    parses "simple application"        "(a 5)"        (AApp () (AIdent () "a") [(AInt () 5)])
    parses "application of a function" "(#([a int] a) 5)" (AApp () (AFunc () [("a", AIntTy)] (AIdent () "a")) [(AInt () 5)])
