...
desugar (AMap m pairs)                = CAssoc m (map (\(k, v) -> (desugar k, desugar v)) pairs)
desugar (AFunc m [] body)             = CThunk m emptyEnv (desugar body)
desugar (AFunc m (param:[]) body)     = CFunc  m emptyEnv (desugarParam param) (desugar body)
desugar (AFunc m (param:params) body) = CFunc  m emptyEnv (desugarParam param) $ desugar (AFunc m params body)
