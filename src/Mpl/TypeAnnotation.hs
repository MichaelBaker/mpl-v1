...
annotate' ctx (CMap _ pairs)       = CMap CMapTy $ map (\(k, v) -> (annotate' ctx k, annotate' ctx v)) pairs
annotate' ctx (CThunk _ body)      = let annBody = annotate' ctx body
                                         in CThunk (CThunkTy $ meta annBody) annBody
annotate' ctx (CFunc _ param body) = let annBody = annotate' (withIdent param ctx) body
                                         in CFunc (CFuncTy (paramType param) (meta annBody)) param annBody
annotate' ctx (CForce _ thunk)     = case annotate' ctx thunk of
                                       t@(CThunk (CThunkTy argTy) _) -> CForce argTy t
                                       t -> CForce CUnknownTy t
annotate' ctx (CApp _ func arg)    = let annArg = annotate' ctx arg
                                         in case annotate' ctx func of
                                              f@(CFunc (CFuncTy a b) _ _) -> if a == meta annArg
