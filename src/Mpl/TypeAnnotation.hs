...
import Mpl.AST   (Core(..), CoreType(..), metaC)
...
check :: Context -> TypeMap -> Core [Int] a -> TypeMap
...
check ctx tm (CThunk path _ body) = let tm' = check ctx tm body
                                        in Map.insert path (CThunkTy $ fromJust $ Map.lookup (metaC body) tm') tm'
check ctx tm (CFunc path _ (param, tyParam) body) = let tm' = check (withIdent param ctx) tm body
                                                        in Map.insert path (CFuncTy (typeOfIdent ctx tyParam) (fromJust $ Map.lookup (metaC body) tm')) tm'
check ctx tm (CTyFunc path _ param body) = Map.insert path CUnknownTy $ check ctx tm body
