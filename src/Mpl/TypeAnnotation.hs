...
check :: (Ord a) => Env -> TypeMap a -> Core a b -> TypeMap a
...
check env tm (CFunc i _ (param, tyParam) body) = let ty  = case typeOf tyParam of Nothing -> CUnknownTy; Just a -> a
                                                     tm' = check (withIdent param ty env) tm body
                                                     in Map.insert i (CFuncTy ty (fromJust $ Map.lookup (metaC body) tm')) tm'
